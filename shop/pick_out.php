...
else
{
    ...
    $picks[] = array('name'=>'<strong>'.$_LANG['goods_type'].':</strong>&nbsp;&nbsp;'.$cat_name, 'url'=>'pick_out.php');
    ...
    if (!empty($_GET['attr']))
    {
        ...
        while ($row = $db->fetchRow($rs))
        {
            $picks[] = array('name'=>'<strong>'.$row['attr_name'].':</strong>&nbsp;&nbsp;'.htmlspecialchars(urldecode($_GET['attr'][$row['attr_id']])), 'url'=>'pick_out.php?cat_id='.$cat_id.search_url($attr_picks, $row['attr_id']));
        }
